...
int main(int argc, char **argv) {  
  ...
  cusparseCheck( cusparseSpMV_bufferSize( cusparseHandle,CUSPARSE_OPERATION_NON_TRANSPOSE,&one, matA,vecX,&zero,vecY,CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT,&bufferSize));
  ...
  for (; it < nsteps; ++it) {
    ...
    cusparseDnVecSetValues(vecX, temp);
    cusparseDnVecSetValues(vecY, tmp);
